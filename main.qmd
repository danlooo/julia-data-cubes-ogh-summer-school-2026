...
- The environment was defined in the file `Project.toml` of the GitHub repository
...
- `YAXArray` is a variable in the NetCDF data model
- stores values of the same element type and dimensions
- Use CF conventions as attributes whenever possible

```{julia}
using YAXArrays
using DimensionalData
data = [10.1 10.2 10.3; 10.4 10.5 10.6]
axes = (X([-180, 180]), Y(range(-90, 90, 3)))
properties = Dict("standard_name" => "air_temperature", "units"=>"degC")
a = YAXArray(axes, data, properties)
```

## Dataset

- `Dataset` is a set of `YAXArrays` that may share dimensions
- can be composed of arrays
...
## Subset Air Temperature

Subset first time point:

```{julia}
start_temperature = temperature[time=1]
```

Subset a specific date:

```{julia}
temperature[time=At(Date("2020-12-21"))];
```

## Plot Map

```{julia}
plot(start_temperature)
```

## Create YAXArrays from scratch

```{julia}
axes = (X(1:3), Y(1:4))
data = rand(3, 4)
properties = Dict("long_name" => "Random test data", "use_case" => missing)
a = YAXArray(axes, data, properties)
```

## Chunking

- Data is usually stored in compressed chunks on disk
- One needs to load the entire chunk even if a single pixel is needed
- One can define chunking on write
- One can optimize for one of the use cases:
  - Request single maps at a given time point
  - Request entire time series for a given pixel
- Sometimes its better to save two copies of the data with different chunking
...
This preserves dimensions yielding a 1×1×2192 array

Convert it into a real vector:

```{julia}
mean_temperature = mean_temperature[longitude=1, latitude=1]
```

## Plot Regional Mean Temperature

```{julia}
title = begin
    temperature.properties["long_name"] *
    ", regional mean (" *
    temperature.properties["units"] * ")"
end

fig, ax, plt = plot(mean_temperature)
ax.title = title
fig
```
